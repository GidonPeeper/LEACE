"""
str_probe_deplabs.py

This script probes to what extent dependency labels are present in word embeddings,
...
Inputs:
    ...
    - Dependency labels for each word (from the original CoNLL-U files, filtered with the same logic as preprocessing)
...
results_dir = "Syntactic_distances/Results/UD/LEACE/SD_on_Deplab_structural/"
...
train_deprel_sentences = extract_filtered_deprel_from_conllu(TRAIN_CONLLU)
test_deprel_sentences = extract_filtered_deprel_from_conllu(TEST_CONLLU)
...
# For dependency labels, we use the mean of the predicted pairwise distances for each word as a feature for classification
...
print("Fitting linear classifier on structural probe features for dependency labels...")
...
results_file = os.path.join(results_dir, f"probe_results_structural_deplab_{emb_name}.txt")
