"""
Generates GPT-2 embeddings with aligned, full SYNTACTIC DISTANCE MATRICES.
This script uses the universal parsing and filtering pipeline to ensure perfect
data alignment with other concept embeddings (e.g., spectral, pos, deplab).
It can process multiple datasets ('narratives', 'ud') via command-line.
"""
import argparse
import pickle
import os
...
# ======================================================================
# Universal Data Loading and Filtering Pipeline (IDENTICAL TO OTHER SCRIPTS)
# ======================================================================

def parse_conllu_universal(file_path):
    # This function is identical to the one in the spectral embedding script.
    sentences = []
    try:
...
def filter_and_validate_sentences(sentences):
    ...
    for s in valid_sentences:
        ...
        if nx.is_connected(G): # This check ensures floyd_warshall will work
...
# ======================================================================
# Concept-Specific Generation: Syntactic Distance Matrix
# ======================================================================
def add_distance_matrices(sentences):
    print("\n--- Step 3: Computing Syntactic Distance Matrices ---")
    processed_sentences = []
    for s in tqdm(sentences, desc="Computing distances"):
        n = len(s['words'])
        G = nx.Graph()
        G.add_nodes_from(range(n))
        for i, head in enumerate(s['heads']):
        ...
        # We already validated connectivity, so no need for try-except here
        dist_matrix = nx.floyd_warshall_numpy(G, nodelist=range(n))
        
        s['distance_matrix'] = torch.from_numpy(dist_matrix.astype(np.float32))
        processed_sentences.append(s)
        
    return processed_sentences

# ======================================================================
# Generic Tokenization and Encoding Functions (Adapted for matrices)
# ======================================================================
def tokenize_and_align(sentences, tokenizer, concept_key):
    ...
    for sentence in tqdm(sentences, desc=f"Tokenizing for '{concept_key}'"):
        words, labels = sentence["words"], sentence[concept_key] # labels is now a [N, N] matrix
        ...
        # Align the matrix by selecting the rows and columns of words that were kept
        # Note: This creates a smaller matrix if some words were dropped during tokenization.
        aligned_labels = labels[np.ix_(original_word_indices, original_word_indices)]
...
# ======================================================================
def main():
    parser = argparse.ArgumentParser(description="Generate GPT-2 embeddings with syntactic distance matrices.")
    parser.add_argument("--dataset", choices=["narratives", "ud"], required=True, help="The dataset to process.")
    args = parser.parse_args()

    CONCEPT_KEY = "distance_matrix"
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    print(f"--- Configuration ---")
    print(f"Dataset: {args.dataset}, Concept: {CONCEPT_KEY}, Device: {device}")
    ...
    final_sentences = add_distance_matrices(golden_sentences)
