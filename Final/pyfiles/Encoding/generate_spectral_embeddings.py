"""
Generates GPT-2 embeddings with aligned, PADDED SPECTRAL GRAPH EMBEDDINGS.
This script can process multiple datasets ('narratives', 'ud') via command-line
arguments and produces a file compatible with the LEACE erasure pipeline.
"""
import argparse
import pickle
import os
...
# ======================================================================
def add_padded_spectral_vectors(sentences):
    print("\n--- Step 3: Computing and Padding Spectral Vectors ---")
    if not sentences:
        print("No valid sentences to process. Exiting.")
        return []
    max_len = max(len(s['words']) for s in sentences)
    print(f"Max sentence length in golden set: {max_len}. Spectral vectors will be padded to dim={max_len-1}.")

    processed_sentences = []
    for s in tqdm(sentences, desc="Computing spectral vectors"):
        n = len(s['words'])
        k = n - 1
        
        G = nx.from_edgelist([(i, h-1) for i,h in enumerate(s['heads']) if h>0], create_using=nx.Graph())
        G.add_nodes_from(range(n)) # Ensure all nodes are present before Laplacian calculation
        L = nx.normalized_laplacian_matrix(G, nodelist=range(n)).toarray()
        eigenvalues, eigenvectors = eigh(L)
        
        spectral_vectors = torch.from_numpy(eigenvectors[:, 1:k+1].astype(np.float32))
        pad_dims = (0, max_len - 1 - k)
        padded_spectral_vectors = torch.nn.functional.pad(spectral_vectors, pad_dims, "constant", 0)

        s['distance_spectral_padded'] = padded_spectral_vectors
...
# ======================================================================
def main():
    parser = argparse.ArgumentParser(description="Generate GPT-2 embeddings with padded spectral vectors.")
    parser.add_argument("--dataset", choices=["narratives", "ud"], required=True, help="The dataset to process.")
    args = parser.parse_args()

    CONCEPT_KEY = "distance_spectral_padded"
    ...
    final_sentences = add_padded_spectral_vectors(golden_sentences)
