...
def main():
    ...
    # File mapping - original files for all concepts and sd-erased
    concept_files = {
    ...
    # Debug print for first sentence
    print("\nDebug info for first sentence:")
    first_sent_sd = data["original_sd"][0]
    first_sent_pos = data["original_pos"][0]
    first_sent_deplabs = data["original_deplabs"][0]
    print("Distance matrix shape:", first_sent_sd.get("distance_matrix", torch.zeros(1)).shape)
    print("POS tags:", first_sent_pos.get("pos_tags", []))
    print("DepLab tags:", first_sent_deplabs.get("deplabs", []))
    print("Embedding shape:", first_sent_sd["embeddings_by_layer"][layer].shape)
    ...
    for emb_type in ["original_sd", "sd"]:
        ...
        # Calculate L2 distance for sd-erased embeddings
        if emb_type == "sd":
            l2 = np.linalg.norm(X_orig - X, axis=1).mean()
            results.loc["sd-erased", "l2 distance to original"] = l2
        ...
        if y_sd:
            ...
            results.loc["original" if emb_type == "original_sd" else f"{emb_type}-erased", "sd probe (r²)"] = r2
            results.loc["original" if emb_type == "original_sd" else f"{emb_type}-erased", "sd probe (pearson r)"] = corr
            results.loc["original" if emb_type == "original_sd" else f"{emb_type}-erased", "sd probe (mse)"] = mse
        ...
        if y_pos:
            ...
            results.loc["original" if emb_type == "original_sd" else f"{emb_type}-erased", "pos probe (r²)"] = r2_pos
            ...
            results.loc["original" if emb_type == "original_sd" else f"{emb_type}-erased", "pos probe (accuracy)"] = acc_pos
        ...
        if y_dep:
            ...
            results.loc["original" if emb_type == "original_sd" else f"{emb_type}-erased", "deplabs probe (r²)"] = r2_dep
            ...
            results.loc["original" if emb_type == "original_sd" else f"{emb_type}-erased", "deplabs probe (accuracy)"] = acc_dep
    ...
    # Save results
    results.to_csv(f"Final/Results/{args.dataset}_{args.method}_sd_probe_results.csv")
