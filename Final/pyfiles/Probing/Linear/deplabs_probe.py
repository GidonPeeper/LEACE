...
def main():
    ...
    # File mapping - original files for all concepts and deplabs-erased
    concept_files = {
    ...
    for emb_type in ["original_deplabs", "deplabs"]:
        ...
        # Calculate L2 distance for deplabs-erased embeddings
        if emb_type == "deplabs":
            l2 = np.linalg.norm(X_orig - X, axis=1).mean()
            results.loc["deplabs-erased", "l2 distance to original"] = l2
        ...
        if y_sd:
            ...
            results.loc["original" if emb_type == "original_deplabs" else f"{emb_type}-erased", "sd probe (r²)"] = r2
            results.loc["original" if emb_type == "original_deplabs" else f"{emb_type}-erased", "sd probe (pearson r)"] = corr
            results.loc["original" if emb_type == "original_deplabs" else f"{emb_type}-erased", "sd probe (mse)"] = mse
        ...
        if y_pos:
            ...
            results.loc["original" if emb_type == "original_deplabs" else f"{emb_type}-erased", "pos probe (r²)"] = r2_pos
            ...
            results.loc["original" if emb_type == "original_deplabs" else f"{emb_type}-erased", "pos probe (accuracy)"] = acc_pos
        ...
        if y_dep:
            ...
            results.loc["original" if emb_type == "original_deplabs" else f"{emb_type}-erased", "deplabs probe (r²)"] = r2_dep
            ...
            results.loc["original" if emb_type == "original_deplabs" else f"{emb_type}-erased", "deplabs probe (accuracy)"] = acc_dep
    ...
    # Save results
    results.to_csv(f"Final/Results/{args.dataset}_{args.method}_deplabs_probe_results.csv")
